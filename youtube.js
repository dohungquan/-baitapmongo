...
// let chanelList = [
//     {
//         link:"https://yt3.ggpht.com/a/AATXAJw1mNTFNiIfjXVfOANE9SHwxBdhKELbEauBBg=s48-c-k-c0xffffffff-no-rj-mo",
//         title:"Trang chủ",
...
// app.get("/youtube-todo", function (req, res) {
//     // console.log(__dirname)

//     // res.sendFile(__dirname +"/index.html")
//     res.render("youtube.ejs", { result: quotes})
// })

// viet ra gia ham moi cap nhat
app.get("/youtube", function (req, res) {
    // console.log(__dirname)
    let todoList = db.collection("newyoutube").find().toArray().then(results => {
        console.log(results)
        res.render("youtube.ejs", { todoList: results,result: quotes })
    }).catch(error => {
        console.error(error)
    })
    // res.sendFile(__dirname +"/index.html")
...
app.get('/youtube/:youtubeId', function (req, res) {
    let id = req.params['youtubeId'];
    let objectId = require('mongodb').ObjectID;
    db.collection("newyoutube").findOne({ _id: new objectId(id) }).then(results => {
        console.log(results)
        res.render("todo-youtube.ejs", { youtube: results })
    }).catch(error => {
        console.error(error)
    })
...
app.post("/new-youtube", function (req, res) {
    ...
    // quotes.push(newTodo);
    db.collection("newyoutube").insertOne(req.body).then(results => {
        console.log(results)
    }).catch(error => {
...
app.post("/update-youtube", function (req, res) {
    ...
    let objectId = require('mongodb').ObjectID;
    db.collection("newyoutube").findOneAndUpdate(
        { _id: new objectId(req.body.youtubeId) },
        { $set: { title: req.body.youtubeTitle, chanel: req.body.youtubeChanel,view: req.body.youtubeView,time: req.body.youtubeTime } },
...
app.post("/delete-youtube", function (req, res) {
    console.log(" Accept request", req.body)
    let objectId = require('mongodb').ObjectID;
    db.collection("newyoutube").deleteOne(
        { _id: new objectId(req.body.youtubeId) },
    ).then(results => {
        console.log(results)
        // res.render("todo.ejs", { todo: results })
...
// app.post("/youtube-new-todo", function (req, res) {
//     console.log("requested", req.body);
//     let newTodo = req.body;
//     quotes.push(newTodo);
// })
...
// app.post("/youtube-delete-todo", function(req, res) {
...
// app.post("/youtube-fix-todo", function(req, res) {
//     console.log("Đã nhận request", req.body)
//     for (var i = 0; i < quotes.length; i++) {
//         if (quotes[i].title === req.body.title) {
